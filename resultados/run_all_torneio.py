...
TEST_SCRIPT = "run_batch_tests_ev_torneio.py"

def run_test_process(batch_size):
    """
    Executes the run_batch_tests_ev_torneio.py script with the given batch_size 
    as the ONLY command-line argument.
    """
    
    # The command we want to run: python run_batch_tests_ev_torneio.py [batch_size]
    ...
    try:
        ...
        result = subprocess.run(
            ...
            check=True, 
            text=True, 
            capture_output=False,  # Set to False to let run_batch_tests_ev_torneio.py print its status directly
            shell=False
        )
        ...
        # This catches a non-zero exit code from run_batch_tests_ev_torneio.py
        duration = time.time() - start_time
...
if __name__ == "__main__":
    ...
    print("Each run will write to its own file (e.g., results_file_5_torneio.txt).")
